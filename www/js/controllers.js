...
.controller('CrearPerfilCtrl', ['$scope', '$location','$state','$filter','$state','$ionicLoading', '$http','$ionicPopup','$ionicHistory','$cordovaCamera','$rootScope','$cookieStore','JsonSvc','$cordovaSQLite',
        ...
        $scope.pierna=[
                   {idpierna:"_none",nombre:""},
                   {idpierna:"derecha",nombre:"Izquierda"},
                   {idpierna:"izquierda",nombre:"Derecha"},
                   {idpierna:"ambidiestro",nombre:"Ambidiestro"}
                   ];
        $scope.creaperfil.pierna = $scope.pierna[0].nombre;
        $scope.posicion=[
                      {idpos:0,nombre:""},
                      {idpos:1,nombre:"Arquero"},
                      {idpos:2,nombre:"Defensa central der"},
                      {idpos:3,nombre:"Defensa central izq"},
                      {idpos:4,nombre:"Lateral derecho"},
                      {idpos:5,nombre:"Volante de corte"},
                      {idpos:6,nombre:"Defensa lateral izq"},
                      {idpos:7,nombre:"Medio ofensivo"},
                      {idpos:8,nombre:"Volante mixto"},
                      {idpos:9,nombre:"Centro delantero"},
                      {idpos:10,nombre:"Armador"},
                      {idpos:11,nombre:"Segundo delantero"}
                   ];
       //$http.defaults.headers.common['X-CSRF-Token'] = $cookieStore.get('key');
       console.log("http://golmap.com/temporal/user/"+sessionStorage.id);
    ...
    JsonSvc.read('http://golmap.com/temporal/getPromociones?tipo_de_promocion=40',$scope)
      .then(function () {
        ...
        {  C[indice] = {
                    ...
                    if ($scope.data.field_posici_n.und[0].value == 0)
                    {
                      positionfut = 0;
                    }else if ($scope.data.field_posici_n.und[0].value == 1)
                    {positionfut = 1;
                    }else if($scope.data.field_posici_n.und[0].value == 2)
                    {positionfut = 2;}
                    else if($scope.data.field_posici_n.und[0].value == 3)
                    {positionfut = 3;}
                    else if ($scope.data.field_posici_n.und[0].value == 4)
                    {positionfut = 4;}
                    else if ($scope.data.field_posici_n.und[0].value == 5)
                    { positionfut = 5;}
                    else if ($scope.data.field_posici_n.und[0].value == 6)
                    { positionfut = 6;}
                    else if ($scope.data.field_posici_n.und[0].value == 7)
                    { positionfut = 7;}
                    else if ($scope.data.field_posici_n.und[0].value == 8)
                    { positionfut = 8;}
                    else if ($scope.data.field_posici_n.und[0].value == 9)
                    { positionfut = 9;}
                    else if ($scope.data.field_posici_n.und[0].value == 10)
                    { positionfut = 10;}
                    else if ($scope.data.field_posici_n.und[0].value == 11)
                    { positionfut = 11;}
                    $scope.crearperfil.posicion = $scope.posicion[positionfut];
                    
                    
                    
                    if ($scope.data.field_pierna_h_bil.und[0].value == '_none')
                    {positionpierna = 0;
                    }
                    else if($scope.data.field_pierna_h_bil.und[0].value == 'derecha')
                    {positionpierna = 1;
                    }else if($scope.data.field_pierna_h_bil.und[0].value == 'izquierda')
                    {positionpierna = 2;
                    }else if($scope.data.field_pierna_h_bil.und[0].value == 'ambidiestro')
                    {positionpierna = 3;}
